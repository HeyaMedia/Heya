...
-- name: SetQueueDJMode :one
UPDATE play_queues
SET dj_mode    = sqlc.arg(dj_mode),
...
-- name: DeleteUpcomingDJQueueItems :execrows
DELETE FROM play_queue_items
WHERE queue_id = sqlc.arg(queue_id)
  AND ord > sqlc.arg(after_ord)
  AND dj_session > 0;
...
-- name: InsertDJQueueItemsAt :execrows
...
WHERE EXISTS (SELECT 1 FROM track_files tf JOIN library_files lf ON lf.id = tf.library_file_id WHERE tf.track_id = t.id AND lf.deleted_at IS NULL)
  AND NOT EXISTS (
      ...
      WHERE existing.queue_id = sqlc.arg(queue_id) AND existing.track_id = t.id
  );
...
-- name: ListQueueTrackIDs :many
SELECT track_id FROM play_queue_items WHERE queue_id = $1 ORDER BY ord;

-- name: MarkQueueItemDJProcessed :exec
UPDATE play_queue_items
SET dj_processed_session = sqlc.arg(dj_session)
WHERE id = sqlc.arg(item_id) AND queue_id = sqlc.arg(queue_id);
...
-- name: ListNextTwoQueueItems :many
SELECT *
...
-- name: FirstUserQueueItemAfter :one
SELECT *
FROM play_queue_items
WHERE queue_id = sqlc.arg(queue_id)
  AND ord > sqlc.arg(after_ord)
