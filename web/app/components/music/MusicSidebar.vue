...
.music-sidebar {
  ...
  flex-shrink: 0;
  /* Same shared --frame-glass-column as the library sidebar (heya.css): the
     flat top holds the topbar's exact --frame-glass, then thins toward the
     bottom where this tall column stands alone over the ambient backdrop —
     one continuous surface from the topbar down. No border-right: any divider
     re-splits the frame into stacked panels; the glass-vs-content contrast
     defines the edge on its own. Blur + Firefox fallback ride the tokens. */
