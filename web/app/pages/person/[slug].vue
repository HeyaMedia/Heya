  ...
  <div v-else-if="data" class="scroll" style="height: 100%">
    <div class="page-pad" style="max-width: 1200px">
      ...
      <div class="person-header">
        ...
        <div class="person-info">
          ...
          <div class="person-meta">
            ...
            <template v-if="data.person.birthday && data.person.place_of_birth">
              <span class="dot" />
            </template>
            <span v-if="data.person.place_of_birth">{{ data.person.place_of_birth }}</span>
            <template v-if="data.person.deathday">
              <span class="dot" />
              <span>Died {{ formatDate(data.person.deathday) }}</span>
            </template>
          </div>
      ...
      <div v-if="data.person.biography" class="person-bio">
        ...
        <p v-for="(para, i) in data.person.biography.split('\n\n').filter(Boolean)" :key="i" class="bio-para">
          {{ para }}
        </p>
      </div>

      <!-- Cast credits (movies/shows they've acted in) -->
      <div v-if="data.cast_credits?.length" class="detail-section">
        ...
        <div class="credits-grid">
          <NuxtLink
            ...
            :to="mediaUrl({ id: c.media_item_id, title: c.title, media_type: c.media_type })"
...
const id = computed(() => parseSlugId(route.params.slug as string))
...
onMounted(async () => {
  if (!id.value) { navigateTo('/'); return }
  try {
    data.value = await apiFetch<PersonResponse>(`/api/person/${id.value}`)
  } catch { /* empty */ }
  loading.value = false
})
</script>
