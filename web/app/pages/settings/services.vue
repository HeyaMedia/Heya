...
async function setExternalPlaylistSync(service: 'listenbrainz' | 'lastfm', externalId: string, enabled: boolean, mode: 'two_way' | 'pull_only' = 'two_way') {
  ...
  try {
    ...
    flash.value = { kind: 'ok', text: enabled ? (mode === 'pull_only' ? 'Playlist imported with automatic updates' : 'Playlist imported and two-way sync enabled') : 'Playlist sync disabled' }
  } catch (e: any) {
...
async function setCollectionAutoSync(service: 'listenbrainz' | 'lastfm', collection: string, enabled: boolean) {
  playlistSyncBusy.value = `${service}:collection:${collection}`
  try {
    await $heya('/api/me/music-services/{service}/playlist-collections/{collection}', {
      method: 'PUT', path: { service, collection }, body: { enabled },
    })
    await loadPlaylistCatalog(service)
    queryCache.invalidateQueries({ key: ['me', 'playlists'] })
    flash.value = { kind: 'ok', text: enabled ? 'Future generated playlists will be imported automatically' : 'Automatic playlist imports disabled' }
  } catch (e: any) {
    flash.value = { kind: 'err', text: e?.data?.detail || 'Could not update automatic playlist imports' }
  } finally {
    playlistSyncBusy.value = ''
  }
}
    ...
    <div v-else class="service-grid">
      <SettingsSection title="ListenBrainz" icon="music" description="Open listening, connected with a personal user token.">
        ...
        <div v-if="lb?.token_set" class="playlist-sync-block">
          ...
          <div v-for="collection in playlistCatalogs.listenbrainz?.collections || []" :key="collection.key" class="playlist-collection">
            <div class="playlist-collection-head">
              <div>
                <strong>{{ collection.name }}</strong>
                <span>{{ collection.description }}</span>
              </div>
              <label class="playlist-auto-toggle">
                <span>Auto-import new playlists</span>
                <AppSwitch
                  :model-value="collection.auto_sync"
                  :disabled="!!playlistSyncBusy"
                  size="md"
                  :aria-label="`Automatically import ${collection.name}`"
                  @update:model-value="setCollectionAutoSync('listenbrainz', collection.key, $event)"
                />
              </label>
            </div>
            <div v-if="!collection.playlists.length" class="playlist-sync-empty">No generated playlists found yet.</div>
            <div v-else class="playlist-sync-list">
              <div v-for="playlist in collection.playlists" :key="playlist.external_id" class="playlist-sync-row">
                <div class="playlist-sync-meta">
                  <a v-if="playlist.url" :href="playlist.url" target="_blank" rel="noopener">{{ playlist.name }}</a>
                  <strong v-else>{{ playlist.name }}</strong>
                  <span>{{ playlist.description || (playlist.local_playlist_id ? 'Pull-only sync enabled' : 'Generated by ListenBrainz') }}</span>
                </div>
                <div class="playlist-sync-actions">
                  <button v-if="playlist.local_playlist_id" class="sv2-btn ghost" :disabled="!!playlistSyncBusy" @click="syncExternalNow('listenbrainz', playlist.external_id, playlist.local_playlist_id)">Refresh</button>
                  <AppSwitch
                    :model-value="!!playlist.local_playlist_id"
                    :disabled="!!playlistSyncBusy || collection.auto_sync"
                    size="md"
                    :aria-label="`Import and update ${playlist.name}`"
                    @update:model-value="setExternalPlaylistSync('listenbrainz', playlist.external_id, $event, 'pull_only')"
                  />
                </div>
              </div>
            </div>
...
.playlist-collection { margin-top: 14px; padding-top: 14px; border-top: 1px solid var(--border); }
.playlist-collection-head { display: flex; align-items: center; justify-content: space-between; gap: 16px; margin-bottom: 9px; }
.playlist-collection-head > div { min-width: 0; display: flex; flex-direction: column; gap: 2px; }
.playlist-collection-head strong { color: var(--fg-0); font-size: 12px; }
.playlist-collection-head span { color: var(--fg-3); font-size: 10.5px; line-height: 1.4; }
.playlist-auto-toggle { display: flex; align-items: center; gap: 8px; flex: none; color: var(--fg-2); font-size: 10.5px; }
...
@media (max-width: 520px) {
  ...
  .playlist-collection-head { align-items: stretch; flex-direction: column; }
  .playlist-auto-toggle { justify-content: space-between; }
}
</style>
