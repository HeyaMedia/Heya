...
<template>
  <div>
    <SettingsContextHero
      ...
      description="Inspect the serving process, host CPU pressure, memory, goroutines, garbage collection, and runtime logging."
    />
    ...
    <template v-else-if="sys">
      <div class="tiles">
        ...
        <MetricTile label="System load" :value="sys.host_cpu_available ? `${sys.host_cpu_percent.toFixed(1)}%` : 'Unavailable'" icon="cpu"
          :tone="sys.host_cpu_percent >= 90 ? 'bad' : sys.host_cpu_percent >= 70 ? 'warn' : 'good'"
          :sub="sys.host_cpu_metric === 'load_average_1m' ? `1m load · ${sys.num_cpu} CPUs` : `${sys.num_cpu} logical CPUs`" />
        <MetricTile label="Serve CPU" :value="`${sys.cpu_percent.toFixed(1)}%`" icon="pulse"
          :tone="sys.cpu_percent >= 100 ? 'warn' : 'good'"
      ...
      <SettingsSection title="Runtime" icon="cpu">
        <KVTable :rows="[
          ...
          { key: 'System load',  value: sys.host_cpu_available ? `${sys.host_cpu_percent.toFixed(1)}% (${sys.host_cpu_metric})` : 'unavailable' },
          { key: 'Serve CPU',    value: `${sys.cpu_percent.toFixed(1)}% (one core = 100%)` },
        ]" />
      </SettingsSection>
      ...
      <SettingsSection title="Log level" icon="clipboard"
        :description="`Active serve-process zerolog level. Boot value (HEYA_LOG_LEVEL) was ${logLevel?.boot_level ?? '—'}; this picker is in-memory only and resets on restart.`">
