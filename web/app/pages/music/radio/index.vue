<template>
  <div class="page-pad radio-page">
    <header class="ri-head">
      <h1 class="ri-title">Internet Radio</h1>
      <p class="ri-sub">{{ stationCountSub }}</p>
      <div class="ri-search">
    ...
    <nav v-if="!searchResults.length" class="ri-strip">
      <NuxtLink to="/music/radio/countries" class="ri-strip-tile">
      ...
      <NuxtLink to="/music/radio/tags" class="ri-strip-tile">
      ...
      <NuxtLink to="/music/radio/favorites" class="ri-strip-tile">
      ...
      <NuxtLink to="/music/radio/recents" class="ri-strip-tile">
...
const topQuery = useQuery({
  queryKey: ['radio', 'top', { category: 'topvote', count: 30 }],
  queryFn: async () => ((await $heya('/api/radio/top', { query: { category: 'topvote', count: 30 } })) as { items: RadioStationView[] }).items ?? [],
...
const tagsQuery = useQuery({
  queryKey: ['radio', 'tags', { limit: 40 }],
  queryFn: async () => ((await $heya('/api/radio/tags', { query: { limit: 40 } })) as { items: Array<{ name: string; stationcount: number }> }).items ?? [],
...
const recentsQuery = useQuery({
  queryKey: ['me', 'radio', 'recents', { limit: 18 }],
  queryFn: async () => ((await $heya('/api/me/radio/recents', { query: { limit: 18 } })) as { items: RecentRow[] }).items ?? [],
  staleTime: 1000 * 30,
})
...
// Search results aren't in vue-query — they're transient, user-typed,
...
.ri-head { margin-bottom: 28px; max-width: 720px; }
.ri-title { font-size: 30px; font-weight: 700; letter-spacing: -0.01em; }
.ri-sub { color: var(--fg-3); font-size: 13px; margin: 4px 0 18px; }
