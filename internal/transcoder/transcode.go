...
func GenerateInitSegment(ctx context.Context, input string, audioTrack int, profile Profile, hwAccel HwAccelConfig, outputPath string) error {
	ctx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()

	var args []string
	args = append(args, "-y", "-nostats", "-loglevel", "warning")
	args = append(args, hwAccel.InputFlags...)
	args = append(args, "-i", input)
	args = append(args, "-t", "0.001")
	args = append(args, "-map", "0:v:0")
	args = append(args, "-map", fmt.Sprintf("0:a:%d", audioTrack))

	if profile.VideoCodec == "copy" || profile.VideoCodec == "" {
		args = append(args, "-c:v", "copy")
	} else {
		encoder := resolveVideoEncoder(profile.VideoCodec, hwAccel)
		args = append(args, "-c:v", encoder)
	}
	if profile.AudioCodec == "copy" || profile.AudioCodec == "" {
		args = append(args, "-c:a", "copy")
	} else {
		args = append(args, "-c:a", profile.AudioCodec, "-b:a", "192k")
	}
