...
func appendRail(rails []RecRail, key, title, subtitle string, items []RecRailItem, minItems int) []RecRail {
	if len(items) < minItems {
		return rails
	}
	return append(rails, RecRail{Key: key, Title: title, Subtitle: subtitle, Items: items})
}
