...
func TestClaimsRootRequest(t *testing.T) {
	tests := []struct {
		...
		{name: "Vite HMR token", path: "/_nuxt/?token=vite-hmr-token", want: false},
		{name: "generic tokenized page", path: "/invite?token=invite-token", want: false},
		{name: "Jellyfin socket token", path: "/socket?token=jellyfin-token", want: true},
