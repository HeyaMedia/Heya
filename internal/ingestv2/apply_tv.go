...
func ApplyTVMaterialization(ctx context.Context, lib sqlc.Library, result Result, db *pgxpool.Pool, emit Emitter) ([]TVApplyResult, error) {
	if db == nil {
		return nil, fmt.Errorf("TV apply db is required")
	}
	...
	for _, preview := range result.TVMaterialize {
		...
		if preview.Action == "blocked" {
			...
			emitTVApplyResult(applied, emit)
			continue
		}
	...
	emitTVApplySummary(results, emit)
	return results, nil
}
...
func replaceTVLibraryFileLinks(ctx context.Context, q *sqlc.Queries, libraryFileID, mediaItemID int64, plan TVPlan, episodeLinks tvEpisodeLinkIndex) error {
	...
	for _, episode := range plan.Episodes {
		...
		if err := createTVLibraryFileLink(ctx, q, libraryFileID, mediaItemID, episodeID, seasonNumber, episodeNumber, 0, plan.Title); err != nil {
			return err
		}
		created++
	}
	...
	if created == 0 {
		for _, absolute := range plan.AbsoluteEpisodes {
			...
			if err := createTVLibraryFileLink(ctx, q, libraryFileID, mediaItemID, episodeID, 0, 0, absoluteNumber, plan.Title); err != nil {
				return err
			}
			created++
		}
...
func emitTVApplyResult(result TVApplyResult, emit Emitter) {
	if emit == nil {
		return
	}
...
func emitTVApplySummary(results []TVApplyResult, emit Emitter) {
	if emit == nil {
		return
	}
	emit.Emit(Event{Event: "materialize.apply_summary", Kind: "tv", Data: tvApplySummary(results)})
}
