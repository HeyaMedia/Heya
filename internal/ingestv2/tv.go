...
func AnalyzeTV(ctx context.Context, inv Inventory, emit Emitter) ([]TVPlan, error) {
	...
	for _, root := range inv.Roots {
		...
		for _, f := range root.Files {
			...
			release := parseTVReleaseFromFile(f)
			emitTVParse(f, release, emit)
			...
			if !ok {
				emit.Emit(Event{
					Event:    "tv.file.unplanned",
					Severity: SeverityWarn,
					Root:     root.Root,
					Path:     f.Path,
					RelPath:  f.RelPath,
					Reason:   "no_show_identity",
					Message:  "file classified as media but no TV show identity could be parsed",
				})
				continue
			}
			...
			if len(episodes) == 0 && len(absoluteEpisodes) == 0 {
				emit.Emit(Event{
					...
					Message:  "file has a TV show identity but no episode or absolute episode number",
					Data: map[string]any{
			...
			emit.Emit(Event{
				Event:   "plan.tv_episode",
				Root:    root.Root,
				Path:    f.Path,
				RelPath: f.RelPath,
				Kind:    "would_materialize_tv_episode",
				Data: map[string]any{
					"title":             plan.Title,
					"year":              plan.Year,
					"source":            plan.Source,
	...
	sortTVPlans(plans)
	emit.Emit(Event{Event: "domain.summary", Data: map[string]any{"domain": "tv", "plans": len(plans)}})
	return plans, nil
}
...
func AnalyzeTVMatches(ctx context.Context, plans []TVPlan, emit Emitter) ([]TVMatch, error) {
	...
	for _, plan := range plans {
		...
		if key == "" {
			emit.Emit(Event{
				Event:    "match.tv.unmatched",
				Severity: SeverityWarn,
				Kind:     "tv",
				Reason:   "no_identity_key",
				Message:  "TV plan has no external id, title/year, or title-only identity key",
				Data: map[string]any{
					"title": plan.Title,
					"year":  plan.Year,
					"files": plan.Files,
	...
	for _, match := range matches {
		event := "match.tv.identity"
		if len(match.Plans) > 1 {
			event = "match.tv.grouped"
		}
		emit.Emit(Event{
			Event: event,
			Kind:  "tv",
	...
	emit.Emit(Event{Event: "match.summary", Data: map[string]any{"domain": "tv", "matches": len(matches), "plans": len(plans)}})
	return matches, nil
}

func parseTVReleaseFromFile(file InventoryFile) *parser.SceneReleaseParse {
	release := parser.ParseSceneReleaseName(file.Name, parser.MediaVideo)
...
func emitTVParse(file InventoryFile, release *parser.SceneReleaseParse, emit Emitter) {
	...
	emit.Emit(Event{Event: "parse.result", Root: file.Root, Path: file.Path, RelPath: file.RelPath, Kind: "tv", Data: data})
}
...
func mergeTVPlan(match *TVMatch, plan TVPlan) {
	...
	for _, episode := range plan.Episodes {
		match.Episodes = append(match.Episodes, TVEpisodeRef{Season: plan.Season, Episode: episode})
	}
	for _, absolute := range plan.AbsoluteEpisodes {
		match.Episodes = append(match.Episodes, TVEpisodeRef{Absolute: absolute})
	}
}
...
var providerIDTagRE = regexp.MustCompile(`(?i)[\[{(]\s*(?:imdb|tmdb|tvdb)[-_:= ]+[a-z0-9]+(?:-[a-z0-9]+)?\s*[\]})]`)
