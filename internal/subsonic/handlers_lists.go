...
func (s *Server) albumList2(w http.ResponseWriter, r *http.Request) ([]AlbumID3, bool) {
	...
	case "starred", "highest":
		...
		page, err := s.app.ListUserRatedAlbums(ctx, u.ID, 9, 10, size, offset)
		if err != nil || page == nil {
...
func (s *Server) starred2Payload(r *http.Request, key string) *Starred2 {
	...
	if page, err := s.app.ListUserRatedArtists(ctx, u.ID, 9, 10, 500, 0); err == nil && page != nil {
		for _, it := range page.Items {
	...
	if page, err := s.app.ListUserRatedAlbums(ctx, u.ID, 9, 10, 500, 0); err == nil && page != nil {
		ids := make([]int64, 0, len(page.Items))
		for _, it := range page.Items {
			ids = append(ids, it.ID)
		}
	...
	if page, err := s.app.ListUserRatedTracks(ctx, u.ID, 9, 10, 500, 0); err == nil && page != nil {
		ids := make([]int64, 0, len(page.Items))
		for _, it := range page.Items {
