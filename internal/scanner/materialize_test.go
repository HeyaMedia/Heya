...
func TestPlanMovieMaterializationRespectsUseLocalData(t *testing.T) {
	result := Result{
		MovieMatches: []MovieMatch{{
			Key:         "tmdb:438631",
			Title:       "Dune",
			...
			Assets:      []MovieAssetPlan{{Type: "poster", RelPath: "Dune (2021)/poster.jpg"}},
		}},
		MovieSearch: []MovieSearchMatch{{
			Accepted:    true,
			Key:         "tmdb:438631",
	...
	previews, err := PlanMovieMaterialization(context.Background(), sqlc.Library{
		ID:        3,
		MediaType: sqlc.MediaTypeMovie,
		Settings:  mustJSONBytes(metadata.LibrarySettings{UseLocalData: false}),
	}, result, store, &captureEmitter{})
	if err != nil {
		t.Fatalf("plan materialization: %v", err)
	}
	if got := previews[0].LocalAssets; got != 0 {
		t.Fatalf("LocalAssets with UseLocalData=false: got %d, want 0", got)
	}

	previews, err = PlanMovieMaterialization(context.Background(), sqlc.Library{
		ID:        3,
		MediaType: sqlc.MediaTypeMovie,
		Settings:  mustJSONBytes(metadata.LibrarySettings{UseLocalData: true}),
	}, result, store, &captureEmitter{})
	if err != nil {
		t.Fatalf("plan materialization: %v", err)
	}
	if got := previews[0].LocalAssets; got != 1 {
		t.Fatalf("LocalAssets with UseLocalData=true: got %d, want 1", got)
	}
}
