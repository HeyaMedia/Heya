...
func registerTaskRoutes(api huma.API, app *service.App) {
	...
	huma.Register(api, adminSecured(op(http.MethodPost, "/api/tasks/{id}/run", "run-task", "Trigger a scheduled task immediately", "Tasks")),
		func(ctx context.Context, in *struct {
			ID string `path:"id" enum:"generate_trickplay,generate_thumbnails,scan_libraries,refresh_stale_items,scan_music_loudness,scan_music_fingerprint,analyze_music_facets" doc:"Task identifier"`
		}) (*StatusOutput, error) {
