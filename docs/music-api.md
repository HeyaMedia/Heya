...
| **Instant Radio** | `POST /api/music/radio` with `{seed: {kind, …}, limit, exclude_track_ids}` — seed kinds: `track`, `artist`, `album`, `text`. Resolves to a seed track ID, runs KNN, then `diversifyByArtist` so the queue doesn't run back-to-back from one artist. Returns `{seed_track_id, tracks}` |
...
| **Playback emission** | `POST /api/me/playback` — **unified** endpoint for video progress AND music scrobbles. Body: `{entity_type, entity_id, position_seconds, total_seconds, completed, source}`. Server dispatches: `movie`/`episode` → upsert `user_watch_progress` (resume state); `track` → append `play_events` (history log). FE calls via `recordPlayback()` in `composables/usePlaybackEvents.ts`; both `useVideoPlayer` and `usePlayer` flow through it. Podcast episodes use a separate `POST /api/me/podcasts/progress` because they don't have media_item IDs. |
