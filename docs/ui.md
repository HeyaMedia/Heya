...
| **`AppRail`** | — (hand-rolled windowing) | **Every horizontal media rail/strip/shelf.** Virtualized fixed-stride sidescroller: only visible tiles (±4 overscan) exist in the DOM, absolutely positioned on a full-width track so the scrollbar is honest; emits `load-more` with an 8-tile runway when `has-more` for endless rails. Props: `items`, `tile-width`/`phone-tile-width`, `gap`/`phone-gap`, `aspect` (track height = image box; captions bleed into the built-in shadow padding) or `tile-height`, `snap`, `memory-key`, `item-key`. Scoped slot `#default="{ item, index }"`. Exposes `scrollByDir(dir, step?)`, `scrollToIndex(i)` (use instead of `scrollIntoView` — off-window tiles aren't in the DOM), `overflows` (gate chevrons/expand chrome on it). Headerless — section chrome stays in the consumer (`ContentRow`, `MusicScrollRow` are the two skins). **Don't hand-roll an `overflow-x` strip** — the shadow-room bleed, phone widths, and scroll memory all live here. |
...
| **`AppHoldButton`** | — (plain button + SVG ring) | A button with a press-and-hold secondary action: quick press emits `click`, holding draws a gold ring that fills over `holdMs` (default 2s) and emits `hold` instead (the release's click is swallowed). All attrs/classes fall through to the `<button>`, so it drops into existing chrome. Used by every rail's left arrow: click steps, hold rewinds to the rail start via `rail.scrollToStart()`. |
